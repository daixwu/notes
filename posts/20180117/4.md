...
Vue 也可以扩展原生的 html 元素，封装可重用的代码

组件的基本组成：样式结构、行为逻辑、数据 [wiki](https://cn.vuejs.org/v2/guide/components.html)
...
```html
<div id="app">
    <my-component v-for="post in posts" :key="post.id" :title="post.title"></my-component>
</div>
